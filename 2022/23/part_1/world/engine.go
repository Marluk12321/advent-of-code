...
func Advance(elves []Position, maxIterations int) {
	...
	for iteration < maxIterations {
		...
		for i, nextElf := range nextElves {
			...
			if positionOccupants[nextElf] == 1 {
...
func CalcEmptyGroundTiles(elves []Position) int {
	if len(elves) == 0 {
		panic(elves)
	}
	firstElf := elves[0]
	...
	util.Debug("minX:", minX, "maxX:", maxX, "\nminY:", minY, "maxY:", maxY)
	lenX := maxX - minX + 1
	lenY := maxY - minY + 1
	return lenX*lenY - len(elves)
}
