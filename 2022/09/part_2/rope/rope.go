...
func MakeRope(size int) Rope {
	return Rope{
		segments: make([]RopeSegment, size),
	}
}

func (rope *Rope) Head() *RopeSegment {
	return &rope.segments[0]
}

func (rope *Rope) Tail() *RopeSegment {
	return &rope.segments[len(rope.segments)-1]
}

func (rope *Rope) Move(direction Direction) {
	rope.Head().move(direction)
	for i := 1; i < len(rope.segments); i++ {
		rope.segments[i].follow(&rope.segments[i-1])
	}
}
