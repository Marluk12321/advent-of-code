...
func getSizeToDelete(filesystem *Filesystem) int {
	rootSize := filesystem.root.getSize()
	freeSpace := TOTAL_SIZE - rootSize
	needToDelete := REQUIRED_SIZE - freeSpace
	fmt.Println("Root size:", rootSize)
	...
	filesystem.root.visit(func(dir *Directory) {
		if size := dir.getSize(); size >= needToDelete && size < bestCandidate {
			bestCandidate = size
			fmt.Println("New candidate:", bestCandidate)
		}
	})
...
func main() {
	...
	fmt.Println("Sum:", getSizeToDelete(&filesystem))
}
