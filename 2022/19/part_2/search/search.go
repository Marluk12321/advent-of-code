...
type Finder struct {
	...
	dominatingStates map[int][]State
	globalBestScore int
}
...
func (finder *Finder) findBestScore(remaining_minutes int, state State) int {
	if remaining_minutes == 0 {
		...
		if score > finder.globalBestScore {
			finder.globalBestScore = score
		}
		return score
	}
	if finder.globalBestScore > finder.theoreticalBestScore(remaining_minutes, state) ||
			finder.seenDominatingState(remaining_minutes, state) {
		return 0
	}
